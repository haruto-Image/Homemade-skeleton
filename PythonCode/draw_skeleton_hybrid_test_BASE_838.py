...
BODY_SKELETON = [
    ...
    # 脚
    (COCO_KP['left_hip'], COCO_KP['left_knee']),
    (COCO_KP['left_knee'], COCO_KP['left_ankle']),
    (COCO_KP['right_hip'], COCO_KP['right_knee']),
...
def draw_skeleton_hybrid(image, all_133_keypoints, conf_threshold=0.3):
    ...
    # 信頼度が高く、かつ、インデックスが17(骨盤)と19(背骨)ではない場合にのみ描画
        if kp[2] > conf_threshold and i not in [PELVIS_IDX, SPINE_IDX]:
