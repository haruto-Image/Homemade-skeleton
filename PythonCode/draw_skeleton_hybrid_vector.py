...
#ベクトル変換
def calculate_vector(p1, p2, conf_threshold=0.1):
    # 2つのキーポイントの信頼度が両方とも閾値を超えているかチェック
    if p1[2] > conf_threshold and p2[2] > conf_threshold:
        # 終点の座標から始点の座標を引く (p1 -> p2 のベクトル)
        vector = p2[:2] - p1[:2]
        return vector
    else:
        # 信頼度が低い場合は計算できない
...
def draw_skeleton_hybrid(image, all_133_keypoints, conf_threshold=0.3):
    ...
    #body_keypointsのそれぞれの関節データは[x座標,y座標,信頼度]で構成されてる
    #body_keypoints[5, 2]はbody_keypointsの5番目かつ信頼度を表している
    ...
    if all_133_keypoints.shape[0] >= 133:
        ...
        #背骨のベクトル計算
        spine_vector = calculate_vector(pelvis, thorax)
        if spine_vector is not None:
            calculated_vectors['spine'] = spine_vector
            # ベクトルが計算できた場合のみ表示
            start_point = tuple(pelvis[:2].astype(int))
            end_point = tuple((pelvis[:2]+spine_vector).astype(int))
            cv2.arrowedLine(image,start_point,end_point,(0,255,255),3)


        # 例2：右腕のベクトルを計算（右肩 -> 右肘）
        right_shoulder = body_keypoints[COCO_KP['right_shoulder']]
        right_elbow = body_keypoints[COCO_KP['right_elbow']]
        right_arm_vector = calculate_vector(right_shoulder, right_elbow)
        if right_arm_vector is not None:
            calculated_vectors['right_arm'] = right_arm_vector
            start_point = tuple(right_shoulder[:2].astype(int))
            end_point = tuple((right_shoulder[:2] + right_arm_vector).astype(int))
            # マゼンタの矢印で描画
            cv2.arrowedLine(image, start_point, end_point, (255, 0, 255), 3)
    
    return image, calculated_vectors
